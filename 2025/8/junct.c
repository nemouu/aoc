...
#define MAX_NO_CONNECTIONS 1000 // Define a maximum junction box connections
...
void merge_branches(Vec nodes[], int index1, int index2) {
...
int main() {
    ...
    {
        if (conns == MAX_NO_CONNECTIONS) { break; }
        merge_branches(vector_arr, pair_arr[i].node1, pair_arr[i].node2);
        conns++;
    }

    // extracting the sizes of the branches
    for (int i = 0; i < file_len; i++) {
        if (vector_arr[i].parent == -1) {
            // printf("Adding size %d from node %d\n", vector_arr[i].size, i);
            sizes[size_count] = vector_arr[i].size;
            size_count++;
        }
    }
    
    // sorting the sizes to obtain the 3 largest ones
    qsort(sizes, size_count, sizeof(int), compare_ints_rev);
